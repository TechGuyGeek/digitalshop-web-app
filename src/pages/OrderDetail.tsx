...
const OrderDetail = () => {
  ...
  const companyName = searchParams.get("companyname") || t("UserOrderDetails");
  ...
  useEffect(() => {
    const loadDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(SERVER_DOMAIN + "menu1/PHPread/CompanyLiveUserOrders/RetriveLiveUserOrderDetails.php", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: formData.toString() });
        const text = await response.text();
        if (!text || text.trim() === "" || text.trim() === "[]") setItems([]);
        else { const parsed = JSON.parse(text); if (Array.isArray(parsed)) setItems(parsed); }
      } catch (err) { console.error("Failed to load order details:", err); } finally { setLoading(false); }
    };
    loadDetails();
  ...
  return (
    <div className="h-screen bg-muted flex flex-col">
      ...
      <div className="bg-card px-4 py-3 border-b border-border shrink-0 space-y-1">
        <div className="flex justify-between text-sm font-semibold text-foreground"><span>{t("Totalitems")}</span><span>{totalItems}</span></div>
        <div className="flex justify-between text-sm font-semibold text-foreground"><span>{t("TOTALPRICE")}</span><span>{totalPrice.toFixed(2)}</span></div>
      </div>
                ...
                {photo ? (
                  <div className="relative w-full h-40">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent px-3 py-2 flex justify-between items-end">
                      <span className="text-white font-bold text-sm uppercase">{name}</span><span className="text-white font-bold text-sm">{price}</span>
                    </div>
                  </div>
                ) : (
                  <div className="px-4 py-3 flex justify-between items-center">
                    <div className="flex items-center gap-2"><Store className="text-muted-foreground" size={18} /><span className="font-bold text-sm text-foreground uppercase">{name}</span></div>
