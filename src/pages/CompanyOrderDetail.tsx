...
import {
  ...
  toggleCompanyOrderFlag, updateCompanyOrderCancellation, withoutOwnerCustomerContact, type CompanyOrderBucket, type CompanyGroupedOrder, type CompanyOrderItem,
} from "@/lib/companyOrders";
import { fetchV1AuthorizedCustomerProfile, V1ApiError } from "@/lib/v1Api";
...
function contactRows(customer: AuthorizedCustomer | null): Array<[string, string]> {
  if (!customer) return [];
  const rows: Array<[string, string]> = [
    ["Address line 1", String(customer.line_one_address || "")],
    ["Address line 2", String(customer.line_two_address || "")],
    ["Address line 3", String(customer.line_three_address || "")],
    ["Address line 4", String(customer.line_four_address || "")],
    ["Country", String(customer.country || "")],
    ["Delivery notes", String(customer.delivery_notes || "")],
  ];
...
export default function CompanyOrderDetail() {
  ...
  const loadDetail = useCallback(async () => {
    ...
    const request = fetchCompanyOrderDetail(bucket, companyId, clientId, dateTime)
      .then(async (nextItems) => {
        const safeItems = nextItems.map(withoutOwnerCustomerContact);
        setItems(safeItems);
        const nextOrder = groupCompanyOrders(safeItems)[0];
        if (!nextOrder?.reference) { setCustomer(null); return; }
        try {
          const result = await fetchV1AuthorizedCustomerProfile(nextOrder.reference);
          setCustomer(result.customer as AuthorizedCustomer);
        } catch {
          // Order history remains visible if the separate authorized contact projection is unavailable.
          setCustomer(null);
        }
      })
  ...
  const links = customer ? buildContactLinks(String(customer.mobile_number || ""), String(customer.email || "")) : { phone: "", sms: "", whatsapp: "", email: "" };
  ...
  return <div className="h-dvh bg-muted flex flex-col">
    ...
    {loading ? <div className="flex-1 flex items-center justify-center"><Loader2 className="animate-spin" /></div> : error ? <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-center"><p className="text-sm text-muted-foreground">{error}</p><Button variant="outline" onClick={() => void loadDetail()}>{t("Refresh") || "Refresh"}</Button></div> : !order ? <div className="flex-1 flex items-center justify-center p-8 text-center text-muted-foreground">{t("NoOrdersToshow")}</div> : <>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        <div className="rounded-xl border border-border bg-card p-4 space-y-3"><div className="flex items-center gap-3"><CustomerOrderImage path={customer?.image_path || order.customerImagePath} alt={customer?.name || order.customerName} className="w-16 h-16 rounded-lg shrink-0" iconSize={32} /><div><p className="font-bold">{customer?.name || order.customerName}</p><p className="text-xs text-muted-foreground">{formatOrderDateTime(order.dateTime)}</p></div></div><OrderStatusRows paymentLabel={`${t("PaymentStatus")}:`} paymentValue={order.hasPaid === "1" ? t("Paid") : t("NotPaid")} deliveryLabel={`${t("DeliveryStatus")}:`} deliveryValue={order.hasDelivered === "1" ? t("Delivered") : t("NotDelivered")} /><div className="flex items-center gap-2 text-sm">{order.customerEmailVerified ? <CheckCircle2 size={16} className="text-green-600" /> : <XCircle size={16} className="text-muted-foreground" />}<span>{order.customerEmailVerified ? "Email verified" : "Email not verified"}</span></div><div className="space-y-1 text-sm">{customer?.email && <p>{customer.email}</p>}{customer?.mobile_number && <p>{customer.mobile_number}</p>}{contactRows(customer).map(([label, value]) => <p key={label}><span className="font-semibold">{label}:</span> {value}</p>)}</div><div className="grid grid-cols-2 gap-3"><Button variant="outline" className="rounded-full" disabled={!links.sms} onClick={() => openContact(links.sms)}><MessageSquare size={16} className="mr-1" />{t("sms")}</Button><Button variant="outline" className="rounded-full" disabled={!links.phone} onClick={() => openContact(links.phone)}><Phone size={16} className="mr-1" />{t("call")}</Button><Button variant="outline" className="rounded-full" disabled={!links.whatsapp} onClick={() => openContact(links.whatsapp)}>WhatsApp</Button><Button variant="outline" className="rounded-full" disabled={!links.email} onClick={() => openContact(links.email)}><Mail size={16} className="mr-1" />{t("Email")}</Button></div>{order.reference && <Button variant="destructive" className="w-full rounded-full" onClick={() => navigate(`/customer-profile-readonly?orderid=${encodeURIComponent(order.reference)}`)}>Manage customer</Button>}</div>
