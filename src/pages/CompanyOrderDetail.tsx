...
const ENDPOINTS: Record<string, string> = {
  today: "RetriveLiveOrderDetails.php",
  week: "RetriveLiveOrderDetailsweek.php",
  month: "RetriveLiveOrderDetailsmonth.php",
};
...
async function fetchOrderDetails(
  ...
  dateTime: string,
  range: string
): Promise<LiveOrderDetailItem[]> {
  const endpoint = ENDPOINTS[range] || ENDPOINTS.today;
  const form = new URLSearchParams();
  form.append("companyID", companyId);
  form.append("OrderResult", clientId);
  form.append("getDateandTime", dateTime);

  const url = SERVER_DOMAIN + "menu1/PHPread/CompanyLiveOrders/" + endpoint;
  console.log("[OrderDetail] fetching", url, { companyId, clientId, dateTime, range });
  ...
  console.log("[OrderDetail] raw response:", text.substring(0, 500));
...
function resolveImage(item: LiveOrderDetailItem): string {
  const raw = item.imagepathx || item.Imagepath || item.imagepath || "";
  if (!raw) return "";
...
const CompanyOrderDetail = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(false);
    fetchOrderDetails(companyId, clientId, dateTime, range)
      .then(setItems)
      .catch(() => {
        setError(true);
        setItems([]);
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {loading ? (
          ...
          items.map((item, idx) => {
            ...
            const price = safePrice(item.OrderPrice);
            return (
              <div
                key={item.ID || idx}
                className="rounded-xl border border-border bg-card p-3 shadow-sm flex gap-3"
              >
                {/* Image */}
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <p className="font-semibold text-foreground text-sm truncate">
                    {item.OrderName || "Unnamed Item"}
                  </p>
                  {item.OrderDesription && (
