...
const Index = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (err instanceof AuthApiError && err.code === "email_delivery_failed") {
        // V1 commits the account before attempting delivery.  Keep the user
        // on the verification path so a resend can recover this partial
        // success instead of incorrectly inviting a duplicate registration.
        setResendEmail(email);
        setResendMessage({
          type: "error",
          text: "Your account was created, but the verification email could not be sent. Please resend it.",
        });
        setResendOpen(true);
        toast.error("Account created; verification email delivery failed.");
      } else {
            ...
            <>
              ...
              <div className="mt-4 flex gap-3">
                <Button
                  ...
                  onClick={() => toast.error("Social sign-in is not available yet. Please use email and password.")}
                >
