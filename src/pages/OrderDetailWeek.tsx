...
const OrderDetailWeek = () => {
  ...
  const companyName = searchParams.get("companyname") || "Weekly Order Details";
    ...
    const loadDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(
          SERVER_DOMAIN + "menu1/PHPread/CompanyLiveUserOrders/RetriveLiveUserOrderDetailsweek.php",
          {
            method: "POST",
        ...
        console.error("Failed to load weekly order details:", err);
        setError(true);
      } finally {
        setLoading(false);
      }
    ...
    <div className="h-screen bg-muted flex flex-col">
      ...
      <div className="bg-primary px-4 py-4 flex items-center gap-3 shrink-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-primary-foreground/70">Weekly Order Details</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="text-sm">Loading weekly order details…</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            <AlertCircle className="mb-4 text-destructive" size={32} />
            <p className="text-sm font-medium">Unable to load weekly order details.</p>
            <p className="text-xs mt-1">Please try again.</p>
          </div>
        ) : items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            <span className="text-4xl mb-4">📋</span>
            <p className="text-sm">No weekly order details found.</p>
          </div>
        ) : (
          items.map((item, idx) => {
