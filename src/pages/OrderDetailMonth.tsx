...
const OrderDetailMonth = () => {
  ...
  const companyName = searchParams.get("companyname") || "Monthly Order Details";
        ...
        if (!text || text.trim() === "" || text.trim() === "[]") {
          ...
          if (Array.isArray(parsed)) {
            const matched = parsed.filter(
              (row: OrderLineItem) => row.DateandTime === dateTime
            );
            setItems(matched.length > 0 ? matched : parsed);
          }
        }
      } catch (err) {
        console.error("Failed to load monthly order details:", err);
        setError(true);
      } finally {
        setLoading(false);
      }
    ...
    <div className="h-screen bg-muted flex flex-col">
      ...
      <div className="bg-primary px-4 py-4 flex items-center gap-3 shrink-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-primary-foreground/70">Monthly Order Details</p>
        </div>
      </div>
      ...
      {!loading && !error && items.length > 0 && (
        <div className="bg-primary px-4 py-3 border-t border-primary-foreground/20 shrink-0 space-y-1">
          <div className="flex justify-between text-sm font-semibold text-primary-foreground">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="text-sm">Loading monthly order details…</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            <AlertCircle className="mb-4 text-destructive" size={32} />
            <p className="text-sm font-medium">Unable to load monthly order details.</p>
            <p className="text-xs mt-1">Please try again.</p>
          </div>
        ) : items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            <span className="text-4xl mb-4">📋</span>
            <p className="text-sm">No monthly order details found.</p>
          </div>
        ) : (
          items.map((item, idx) => {
