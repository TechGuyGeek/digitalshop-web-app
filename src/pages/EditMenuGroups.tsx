...
const EditMenuGroupsPage = () => {
  ...
  const actuallyAddGroup = async (name: string) => {
    setAddingGroup(true);
    const result = await addMenuGroup(companyId, name, 0, "", "");
  ...
  const handleDelete = async (group: MenuGroup) => {
    ...
    const result = await deleteMenuGroup(group.ID, companyId, group.OrderGroup, 0, "", "");
    setDeleteConfirm(null);
  ...
  const handleToggle = async (group: MenuGroup, enabled: boolean) => {
    ...
    const ok = await toggleMenuGroupEnabled(group.ID, newVal, companyId, 0, "", "");
    if (!ok) {
      setGroups((prev) =>
        prev.map((g) =>
  ...
  const handleSaveEdit = async () => {
    ...
    setSavingEdit(true);
    const result = await updateMenuGroup(editGroup.ID, editGroup.OrderGroup, newName, 0, "", "");
    setSavingEdit(false);
    if (result.success) {
      toast.success(t("SaveSuccessful"));
      setEditGroup(null);
