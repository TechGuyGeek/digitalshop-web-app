...
const AdminShops = () => {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      catch { setError(`Invalid response: ${text.slice(0, 120)}`); setLoading(false); return; }
      if (!Array.isArray(companies)) { setError("Unexpected response format"); setLoading(false); return; }
  ...
  const handleShopMapClick = (shop: { name: string; icon: string; companyid?: number }) => {
    if (shop.companyid) navigate(`/shop-profile?companyid=${shop.companyid}&name=${encodeURIComponent(shop.name)}&icon=${encodeURIComponent(shop.icon)}`);
  };

  return (
    <div className="min-h-screen bg-background flex flex-col">
      <div className="bg-primary px-4 py-4 flex items-center gap-3">
        <Button variant="ghost" size="icon" className="text-primary-foreground hover:bg-primary/80" onClick={() => navigate("/")}><ArrowLeft size={20} /></Button>
          ...
          <div className="divide-y divide-border">
            {shops.length === 0 && (
              <div className="flex-1 flex items-center justify-center p-8">
                <div className="text-center"><MapPin size={32} className="mx-auto mb-3 text-muted-foreground" /><p className="text-sm text-muted-foreground">No shops returned</p></div>
              </div>
            )}
            {shops.map((shop) => (
              <button key={shop.companyid} className="w-full flex items-center gap-4 px-6 py-4 text-left hover:bg-accent/50 transition-colors" onClick={() => navigate(`/shop-profile?companyid=${shop.companyid}&name=${encodeURIComponent(shop.name)}&icon=${encodeURIComponent(shop.icon)}`)}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-xs text-muted-foreground">{shop.categoryLabel} · ID {shop.companyid}</span>
                </div>
              </button>
            ))}
          </div>
