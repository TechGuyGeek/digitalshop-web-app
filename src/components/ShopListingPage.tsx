  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex flex-col">
        {activeTab === "hybrid" && (<><GoogleMap className="h-56 w-full" shops={mapShops} onShopClick={handleShopMapClick} defaultZoom={isGlobal ? 3 : 14} /><ShopContent shops={shops} loading={loading} error={error} isGlobal={isGlobal} onRetry={() => isGlobal ? loadShops() : userPos && loadShops(userPos.lat, userPos.lng)} /></>)}
        {activeTab === "map" && (<><GoogleMap className="flex-1 min-h-[400px] w-full" shops={mapShops} onShopClick={handleShopMapClick} defaultZoom={isGlobal ? 3 : 14} />{loading && (<div className="p-4 text-center text-sm text-muted-foreground">{t("Pleasewait")}</div>)}</>)}
