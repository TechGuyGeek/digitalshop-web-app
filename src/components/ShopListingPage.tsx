  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex flex-col">
        {activeTab === "hybrid" && (<><GoogleMap className="h-56 w-full" shops={mapShops} onShopClick={handleShopMapClick} defaultZoom={isGlobal ? 3 : 14} rangeCircleMetres={variant === "free" ? 804.67 : undefined} />{variant === "free" && <p className="text-[11px] text-muted-foreground text-center py-1">{t("Showingshopswithin")||"Showing shops within 0.5 miles"}</p>}<ShopContent shops={shops} loading={loading} error={error} isGlobal={isGlobal} onRetry={() => isGlobal ? loadShops() : userPos && loadShops(userPos.lat, userPos.lng)} /></>)}
        {activeTab === "map" && (<><GoogleMap className="flex-1 min-h-[400px] w-full" shops={mapShops} onShopClick={handleShopMapClick} defaultZoom={isGlobal ? 3 : 14} rangeCircleMetres={variant === "free" ? 804.67 : undefined} />{variant === "free" && <p className="text-[11px] text-muted-foreground text-center py-1">{t("Showingshopswithin")||"Showing shops within 0.5 miles"}</p>}{loading && (<div className="p-4 text-center text-sm text-muted-foreground">{t("Pleasewait")}</div>)}</>)}
