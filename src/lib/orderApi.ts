...
export async function getMyOrder(id:string) { return (await read<{order:V1Order}>(await authenticatedFetch(`${API}?id=${encodeURIComponent(id)}`))).order; }
export async function getOwnedOrder(id:string) { return (await read<{order:V1Order}>(await authenticatedFetch(`${API}?id=${encodeURIComponent(id)}&owner=true`))).order; }
export async function requestOrderCancellation(id:string) { return read<{requested:boolean}>(await authenticatedFetch(`${API}?id=${encodeURIComponent(id)}&action=cancel`,{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"})); }
export async function updateOwnedOrder(id:string,input:{paid?:boolean;delivered?:boolean;cancellation_status?:"approved"|"rejected"}) { return read<{updated:boolean}>(await authenticatedFetch(`${API}?id=${encodeURIComponent(id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(input)})); }
export async function deleteMyOrder(id:string) { return read<{deleted:boolean}>(await authenticatedFetch(`${API}?id=${encodeURIComponent(id)}`,{method:"DELETE"})); }
